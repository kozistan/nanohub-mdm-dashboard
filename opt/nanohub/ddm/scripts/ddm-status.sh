...
show_device() {
    ...
    db_result=$(docker exec -i mysql-nanohub mysql -u nanohub -p'your_db_password' nanohub -N -e "
...
" 2>/dev/null || echo "  Error parsing status"
    ...
    echo "Status errors:"
    error_result=$(docker exec -i mysql-nanohub mysql -u nanohub -p'your_db_password' nanohub -N -e "
