...
COMMANDS = {
    ...
    'lost_mode': {
        'name': 'Lost Mode',
        'category': 'security',
        'description': 'Lost Mode for supervised iOS/iPadOS (enable / locate / disable)',
        'script': '_internal_lost_mode',
        'parameters': [
            {'name': 'udid', 'label': 'Device', 'type': 'device', 'required': True},
            {'name': 'action', 'label': 'Action', 'type': 'select', 'required': True,
             'options': [
                 {'value': 'enable', 'label': 'Enable Lost Mode (lock + auto-request location)'},
                 {'value': 'locate', 'label': 'Request Location again (needs Lost Mode active)'},
                 {'value': 'disable', 'label': 'Disable Lost Mode (unlock)'},
             ]},
            {'name': 'reason', 'label': 'Reason (audit)', 'type': 'string', 'required': True,
             'placeholder': 'ticket # / why'},
            {'name': 'message', 'label': 'Message (enable only)', 'type': 'string', 'required': False,
             'placeholder': 'This device has been lost. Please contact...'},
            {'name': 'phone', 'label': 'Phone (enable only)', 'type': 'string', 'required': False,
             'placeholder': '+420...'},
            {'name': 'footnote', 'label': 'Footnote (enable only)', 'type': 'string', 'required': False,
             'placeholder': 'Optional lock-screen footnote'},
        ],
        'dangerous': True,
        'danger_level': 'high',
        'min_role': 'admin',
